...
import { ValidationError, SelfSendNotAllowedError, BotTargetNotAllowedError, PointTypeNotFoundError, PointTypeDisabledError, DailyLimitExceededError, } from './errors.js';
...
export class PointsService {
    ...
    async give(params) {
        ...
        const todayCount = await this.repo.countGiverTransactionsInRange(params.guildId, params.typeKey, params.giverUserId, fromIso, toIso);
        ...
        // 残高を更新
        const newBalance = await this.repo.upsertBalance(params.guildId, params.receiverUserId, params.typeKey, params.amount);
        // 6) 戻り値
        return { newBalance, txId };
    }
    async getBalances(guildId, userId, typeKey) {
        if (typeKey) {
    ...
    async getLeaderboard(guildId, typeKey, limit = 10) {
